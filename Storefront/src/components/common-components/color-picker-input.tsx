...
const ExpColorPickerInput: React.FC<any> = ({
  changeHandler,
  field,
  inputValue,
  type,
  ...
  const value = inputValue ? inputValue : (defaultValue?.value ||  '');
  ...
  const changeColor = (colorHex: any, setFromInput?: boolean) => {
    if (setFromInput === true) {
      if (!colorHex?.length) {
        changeHandler(field, '#000000', type);
      } else {
...
export default ExpColorPickerInput;
